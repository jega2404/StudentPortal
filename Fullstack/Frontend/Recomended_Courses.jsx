...
import 'react-circular-progressbar/dist/styles.css';
import Badge1 from '../assets/images/i.png'
import Badge2 from '../assets/images/i2.png'
import Badge5 from '../assets/images/i3.png'
    ...
    <>
      ...
      <Modal finalFocusRef={finalRef} isOpen={isOpen} onClose={onClose} size='2xl'  >
        ...
        <ModalContent>
            ...
            <Table variant="simple">
              <Thead>
                ...
                <Tr>
                  ...
                  <Td className='text-gray-400'>Jega</Td>
          ...
          <ModalFooter backgroundColor='bg-cyan-400'>
...
function Course() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8989/getAllCourses')
  ...
  useEffect(() => {
    ...
    console.log(id)
    axios.get('http://localhost:8989/getEnroll', { params: { id: id } })
      .then((response) => {setEnrolledCourses(response.data);console.log(response.data)})
...
// console.log(enrolledCourses)
  ...
  const [percentageBasic, setPercentageBasic] = useState(0);
  const [percentageIntermediate, setPercentageIntermediate] = useState(0);
  const [percentageHard, setPercentageHard] = useState(0);
  const [minuteMonthly, setMinuteMonthly] = useState(0);
  const [secondMonthly, setSecondMonthly] = useState(0);

  useEffect(() => {
    let totalBasicCount = 0;
    ...
    let percentageBasicCount = 0;
    let percentageIntermediateCount = 0;
    let percentageHardCount = 0;
    let minuteMonthlyCount = 0;
    let secondMonthlyCount = 0;

    for (let i = 0; i < allCourses.length; i++) {
      const courseLevel = allCourses[i].level;
    ...
    for (let j = 0; j < enrolledCourses.length; j++) {
      const courseLevel = enrolledCourses[j].courses.level;
      const status = enrolledCourses[j].status;
      if (courseLevel === 'Basic' && status === 'completed') {
        BasicCount++;
      } else if (courseLevel === 'Intermediate' && status === 'completed') {
        IntermediateCount++;
      } else if (courseLevel === 'Advanced' && status === 'completed') {
        HardCount++;
      }
    }

    completedCount = BasicCount + IntermediateCount + HardCount;
    totalCount = totalBasicCount + totalIntermediateCount + totalHardCount;

    if (totalCount > 0) {
      percentageCount = (completedCount / totalCount) * 100;
    }

    if (totalBasicCount > 0) {
      percentageBasicCount = Math.round((BasicCount / totalBasicCount) * 100);
    }

    if (totalIntermediateCount > 0) {
      percentageIntermediateCount = Math.round((IntermediateCount / totalIntermediateCount) * 100);
    }

    if (totalHardCount > 0) {
      percentageHardCount = Math.round((HardCount / totalHardCount) * 100);
    }

    minuteMonthlyCount = 180 * 2.1;
    secondMonthlyCount = 24;

    // Update state variables
    setTotalBasic(totalBasicCount);
    setTotalIntermediate(totalIntermediateCount);
    setTotalHard(totalHardCount);
    setBasic(BasicCount);
    setIntermediate(IntermediateCount);
    setHard(HardCount);
    setCompleted(completedCount);
    setTotal(totalCount);
    setPercentage(percentageCount);
    setPercentageBasic(percentageBasicCount);
    setPercentageIntermediate(percentageIntermediateCount);
    setPercentageHard(percentageHardCount);
    setMinuteMonthly(minuteMonthlyCount);
    setSecondMonthly(secondMonthlyCount);

  }, [allCourses, enrolledCourses]);

  console.log(completed, percentage, percentageBasic, percentageIntermediate, percentageHard);
  console.log(enrolledCourses);

  const setstatus = async (enrollid) => {

    console.log("H", enrollid)
    const selectedEnroll = enrolledCourses.find((course) => course.enrolledId === enrollid);
    selectedEnroll.status = "completed";
    //  setEnrolledCourses((prevEnroll) => ({
    //   ...prevEnroll,
    //   status:'completed'
    // }));
    // console.log(enrolledCourses);
    console.log(selectedEnroll);
    await axios.post('http://localhost:8989/user/postEnroll', selectedEnroll).then((response) => {
  ...
  return (
    <div className='flex flex-row'>
      <div className='fixed z-50'>
        <SideBar links={{ studentlinks: studentlinks, currentlinks: 'Dashboard', role: 'user' }} />
      </div>

      <div className='bg-gray-700 ml-40 h-full w-full'>
        <div className='bg-gray-800 text-3xl p-2 m-2 text-white rounded font-bold'>Welcome back <span className='text-teal-700'>{localStorage.getItem('email')}</span></div>
        ...
        {enrolledCourses.length != 0 ? (<>
          <SimpleGrid spacing={2} columns={[1, 2, 3, 4, 5]} mt='5' >
            {enrolledCourses.map(course => (
              <Card key={course.courses.course_id} maxH={'xs'} backgroundColor='gray.800'>
                <CardHeader>
                  <Heading size='md' className='text-white'>{course.courses.course_name}</Heading>
                </CardHeader>
                <CardBody>
                  <Image
                    mx="auto"
                    ...
                    maxH="100px"
                    src={course.courses.imageUrl}
                    alt={`Course ${course.enrolled_id}`}
                    borderRadius='lg'
                  />
                ...
                <Text fontSize='x-small' textAlign='left' ml='2' className='text-white'>status : <span className={course.status === "completed" ? 'text-teal-500' : 'text-red-500'}
                >{course.status}</span></Text>
                <Box mt={2} m={2}>
                ...
                >
                  {/* {console.log(course.enrolledId)} */}
                  <FaPlayCircle size='20' cursor='pointer' onClick={() => { alert('h'), setstatus(course.enrolledId) }}></FaPlayCircle>

                  <Text fontSize='xs' color='gray.500' alignSelf='end' ml='3' display='flex'>
                    <span className='text-white'> Duration : </span>  <span className='text-gray-100 font-bold ml-1'> {course.courses.duration}</span>
                  </Text>
                </CardFooter>
              </Card>
            ))}
